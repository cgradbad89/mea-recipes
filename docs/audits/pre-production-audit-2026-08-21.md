...
| M-05 | **Medium** | Several AI routes do not validate request shapes or bound prompt payloads, and several return raw exception messages to clients. | `app/api/new-recipe-suggestions/route.ts:14-26,59-62`; `app/api/recommendations/route.ts:12-50,121-124`; `app/api/recipe-assistant/route.ts:9-55`; `app/api/ai-ingest/route.ts:44-49,78-120,138-141` |
...
| L-01 | **Low** | Favorites retain prior signed-in state after sign-out when no anonymous localStorage key exists; the known pre-load toggle race remains harmless/idempotent. | `components/AppDataProvider.tsx:105-151`; `lib/userdata.ts:23-47` |
...
| L-03 | **Low** | Operational docs contradict code: README says MFP is manual-only despite a daily cron; the PRD API table says canonical dry-run has no apply path despite live apply code. | `README.md:124-129`; `vercel.json:7-11`; `PRD.md:87`; `app/api/nutrition-canonical-dryrun/route.ts:91-165` |
...
| L-05 | **Low** | A dead Strava helper still reads the unowned root collection; active nutrition code correctly uses uid-scoped health metrics. | `lib/strava.ts:12-46`; `lib/healthMetrics.ts:23-46`; `app/nutrition/page.tsx:23,131-145` |
...
- **FAIL — M-05, request inputs are mostly trusted.** Examples: new suggestions calls `.join`/`.slice` on unvalidated request properties (`app/api/new-recipe-suggestions/route.ts:19-26`); recommendations assumes arrays/maps and repeatedly calls `.find` (`app/api/recommendations/route.ts:17-50`); recipe assistant forwards an unbounded messages array (`app/api/recipe-assistant/route.ts:9-45`); pasted AI-ingest HTML/text is not byte-bounded before prompt construction (`app/api/ai-ingest/route.ts:49,78-120`). These produce avoidable 500s and allow excessive model input/cost from any authenticated account.
- **FAIL — internal messages leak.** Several outer catch blocks return `err.message` directly (`app/api/grocery-cleanup/route.ts:104-106`; `app/api/new-recipe-suggestions/route.ts:59-62`; `app/api/recommendations/route.ts:121-124`; `app/api/recipe-assistant/route.ts:52-56`). Log server detail with a request ID and return a stable public error code/message instead.
...
- **New low-severity stale-state issue.** On sign-out, `refetchFavorites` replaces state only if an anonymous localStorage key exists. With no key, the previous signed-in set remains in memory (`components/AppDataProvider.tsx:110-117`). Always set either the parsed anonymous set or `new Set()` and clear the prior error on identity change.
...
- **Strava changed/resolved on the active path.** Nutrition now queries `users/{uid}/healthMetrics` (`lib/healthMetrics.ts:28-46`; `app/nutrition/page.tsx:23,131-145`). The prior root `stravaActivities` query remains only in an unreferenced helper that assumes all documents belong to the current user (`lib/strava.ts:12-46`). Remove/deprecate it so it cannot be accidentally reintroduced. The production seven-day error log contains no matching permission errors.
- **MFP runtime design is guarded.** The route checks a secret, validates both diary pages before any Firestore mutation, maps columns by header, uses deterministic IDs, and commits delete+replace atomically (`app/api/cron/sync-nutrition/route.ts:119-223,294-352`). The fail-closed secret fix is H-02. Documentation incorrectly says the route is manual-only (`README.md:127`) while Vercel schedules it daily at 06:00 UTC (`vercel.json:7-11`).
...
- [x] **Strava permission failure on every page load:** **changed/resolved on the active path** — nutrition uses uid-scoped health metrics; dead root helper remains (`lib/healthMetrics.ts:28-46`; `lib/strava.ts:12-46`).
...
- Core data/integration: `lib/firebase.ts`, `lib/firebaseAdmin.ts`, `lib/ai.ts`, `lib/aiConfig.ts`, `lib/nutritionEngine.ts`, `lib/canonicalStaples.ts`, `lib/recipes.ts`, `lib/userdata.ts`, `lib/queue.ts`, `lib/consumptionLog.ts`, `lib/healthMetrics.ts`, `lib/strava.ts`, `lib/ingredientParser.ts`, `lib/groceryCleanup.ts`, `lib/recipeContent.ts`.
