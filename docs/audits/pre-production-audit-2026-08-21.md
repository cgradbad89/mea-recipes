...
| M-05 | **Medium — fixed** | The four affected AI routes now enforce streaming raw-body ceilings, explicit request schemas, route-specific semantic bounds, AI/fetch short-circuiting on invalid input, and sanitized public failures. | `lib/apiRequest.ts`; `app/api/new-recipe-suggestions/route.ts`; `app/api/recommendations/route.ts`; `app/api/recipe-assistant/route.ts`; `app/api/ai-ingest/route.ts`; focused route tests |
...
- **FIXED — M-05 internal messages are sanitized on all four affected routes.** Expected input failures have stable 400/413 responses, existing 401 auth behavior is preserved, and provider/Firebase/unexpected exception text is absent from public 500 responses. Server logs retain stable route/operation identifiers and safe counts/lengths without logging full bodies or bearer tokens. The separate raw-message catch in `/api/grocery-cleanup` remains outside M-05's explicitly affected route set and was not changed in this work.
