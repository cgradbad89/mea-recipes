#!/usr/bin/env node
/**
 * Exact six-recipe Wave 2 content remediation.
 *
 * Dry-run creates the immutable manifest without using any Firestore write API.
 * Apply accepts only that exact manifest SHA and updates only recipes/{id}.content.
 */
...
async function dryRun(db, parseRecipeContent) {
  ...
  if (corpusSafety.mappedRecipes !== 187 || corpusSafety.storedHashMismatches.length !== 0) {
    throw new Error(`Mapped-corpus gate failed: ${stableJson(corpusSafety)}`)
  }
  const manifest = buildManifest({
    liveRecipes: exactSix(allRecipes),
    auditRows,
    parseRecipeContent,
    createdAt: new Date().toISOString(),
  })
  if (manifest.counts.READY + manifest.counts.SKIP !== 6) throw new Error('Incomplete six-recipe manifest')
...
async function apply(db, parseRecipeContent) {
  ...
  // Immediate complete live reread supplies both row preconditions and the unaffected-corpus baseline.
  ...
  if (beforeMapped.mappedRecipes !== 187 || beforeMapped.storedHashMismatches.length !== 0) {
    throw new Error(`Mapped-corpus pre-apply gate failed: ${stableJson(beforeMapped)}`)
  }
  const plan = buildApplyPlan({ manifest, liveById: exactSix(beforeAll), parseRecipeContent })
  ...
  try {
    ...
    applyResult = {
      batchCount: plan.readyToWrite.length ? 1 : 0,
  ...
  // Mandatory full readback even after an ambiguous commit response.
  const afterAll = await readAllRecipes(db)
  const afterSix = exactSix(afterAll)
  const readback = verifyReadback({ manifest, plan, liveById: afterSix, parseRecipeContent })
  if (commitError && readback.exactContentMatches === plan.readyToWrite.length) {
    applyResult.committedWrites = plan.readyToWrite.length
  }
  const corpusChanges = verifyCorpusChanges({
    beforeById: beforeAll,
    afterById: afterAll,
    writtenRecipeIds: applyResult.writtenRecipeIds,
  })
  ...
  const postPlan = buildApplyPlan({ manifest, liveById: afterSix, parseRecipeContent })
  const repairedRows = manifest.rows.filter(row => {
    const live = afterSix.get(row.recipeId)
  ...
  const criticalFailure = Boolean(commitError) ||
    ...
    readback.nonContentMismatches > 0 ||
    readback.mapFieldsPresent > 0 ||
    corpusChanges.outsideAuthorizedMutations.length > 0 ||
    corpusChanges.mappedRecipeMutations.length > 0 ||
    corpusChanges.persistedMapChanges.length > 0 ||
    corpusChanges.nonContentMutations.length > 0 ||
    mappedImpact.ingredientArrayChanges.length > 0 ||
    mappedImpact.instructionArrayChanges.length > 0 ||
    mappedImpact.sourceHashChanges.length > 0 ||
    afterMapped.mappedRecipes !== 187 ||
    afterMapped.storedHashMismatches.length > 0 ||
    postPlan.readyToWrite.length > 0 ||
    postPlan.errors.length > 0
  const executiveResult = criticalFailure
    ? 'FAIL'
    : plan.skipped.length > 0 && repairedRows < 6
      ? 'PASS WITH SKIPS'
      : 'PASS'
  const report = {
    schemaVersion: 1,
    ...
    corpusSafety: {
      ...
      details: corpusChanges,
      mappedImpact,
    },
    postApply: {
      readyToWrite: postPlan.readyToWrite.length,
      skipped: postPlan.skipped.length,
      skipRows: postPlan.skipped,
      errors: postPlan.errors,
    },
    repairedRows,
    remainingExcludedRecipes: 21 - repairedRows,
    mapFieldsWritten: 0,
    aiCalls: 0,
  ...
  if (criticalFailure) throw new Error('Wave 2 apply/readback verification failed; see durable apply report')
  return report
}
