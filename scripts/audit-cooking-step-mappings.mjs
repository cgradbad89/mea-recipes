...
function renderReport(date, raw, manifest, reviews, manifestIntegrity) {
  ...
  const lines = [
    ...
    `| No parsed ingredients | ${count(raw.rows, row => row.sourceStatus === 'EXCLUDE_NO_INGREDIENTS')} |`,
    `| No parsed instructions | ${count(raw.rows, row => row.sourceStatus === 'EXCLUDE_NO_INSTRUCTIONS')} |`,
