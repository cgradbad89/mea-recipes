...
admin-verified `/api/mapping-review/**` API routes — but **Cooking Mode production runtime still
does not**: `components/CookingMode.tsx` and existing v4/v5 map behavior are completely unchanged,
and the current-approved pointer this UI can now populate is not yet read by anything at request
time. Runtime cutover remains a later, separately gated task. All writes remain admin-only (same
trusted boundary as other shared-recipe writes), via the Admin SDK only — see **Firestore rules**
below.
