...
| Cooking Mode recall remediation | High | V10G full frozen-benchmark validation FAILED its pre-committed gate; deterministic active-object refinement is stopped and AI-at-ingestion architecture must be reassessed next | V9 failed at four independent layers. V10A froze 863 ingredient candidates (833 correct/30 incorrect); disagreement-only reached 831 TP / 20 FP / 2 FN with 91 AI decisions. V10B measured 748 TP / 9 FP / 85 FN and exposed nine invalid quantity rejections. V10C repaired all 9/9 quantity regressions and rejected 18/20 target FP with clean 71/71 transport, but over-rejected correct candidates: 669 TP / 2 FP / 164 FN, 99.70% precision, 80.31% recall; historical locked protection was 9/12. V10D confirmed the two ratatouille salt/pepper false accepts are benchmark-consistent (not a labeling error — salt/pepper's `CORRECT` link is only the row's first-combination instruction), then added a truth-blind principal-target/alias/timeline/generic-seasoning contract (continuation requires zero prior established component membership). The corrected bounded run (392 risk + 82 historical candidates, 71/71 successful across two full passes, zero retries/failures) reached 642 TP / 0 FP / 191 FN — 100% precision, 20/20 target-FP protection, 9/9 quantity repairs kept — but only 77.07% candidate recall (gate 99.5%) and historical locked protection fell to 4/12. V10E (2026-08-28, read-only discovery, no new experiment) reconstructed and reclassified all 191 V10D false negatives into 10 source-grounded classes with zero `OTHER`-style fallback rows: dominant `DISH_STATE_CONTINUATION` (58/191, 30%), `PRONOUN_OR_DEICTIC_REFERENCE` (55/191, 29%), `SERVING_OR_GARNISH_ACTION` (20/191, 10%). Root cause isolated: 31/58 of the dominant class already carry non-empty componentMembership at rejection time, and 14 componentKey labels recur on both the FN population and the locked historical-FP-risk population, so V10D's "zero prior membership" gate cannot discriminate on membership *existence* — it needs membership *kind* (generic bulk vs. isolated sub-component). Benchmark truth is consistent; the detection heuristic is not. Verdict `REMAINING_SEMANTIC_CLASS_ISOLATED`; recommended next subsystem is a per-recipe-instance active-object graph. **V10F-Lite (2026-08-28, small bounded experiment)** tested the narrowest version of that concept against a 51-case deterministic challenge set (20 positives, 11 negative leakage, 20 locked target FP): 90.00% `DISH_STATE_CONTINUATION` recovery, 100.00% `PRONOUN_OR_DEICTIC_REFERENCE` recovery, 100% precision (0/31 false accepts), 20/20 target-FP protection, 6 flat rules, 0 special cases. Verdict **GO — SIMPLE ACTIVE-OBJECT RESCUE WARRANTS FULL FROZEN VALIDATION**. **V10G (2026-08-28, final full-frozen validation)** ran the same byte-identical rule (SHA-256 verified unchanged) rescue-only against all 863 frozen candidates: 773 TP / 0 FP / 60 FN, 100% precision, 92.80% recall (+15.7 points over V10D), 0 new false positives across all 30 incorrect candidates, 20/20 target-FP and 9/9 quantity-regression protection held, ratatouille policy held. `DISH_STATE_CONTINUATION` recovered 96.55% (56/58, clearing the 90% preferred bar) but `PRONOUN_OR_DEICTIC_REFERENCE` recovered only 83.64% (46/55) on the full population versus 100% on the 10-case challenge subset — the small challenge set did not generalize on this dominant class. Cause: the rule's establishing-instruction check requires an earlier instruction to literally name an ingredient's own key token, which fails when a recipe narrates several distinct ingredients under one generic collective word (e.g. "spices" standing in for cumin/basil/thyme/oregano) instead of naming each one — a token-matching ceiling, not a benchmark defect. Per the pre-committed gate (both dominant classes must clear 85%), verdict **FAIL — ACTIVE-OBJECT RESCUE DOES NOT GENERALIZE**; recommendation **STOP ACTIVE-OBJECT DETERMINISTIC REFINEMENT — REASSESS AI-AT-INGESTION ARCHITECTURE** (for scale: the original two-reviewer union recovered ~99.93% of confirmed omissions). The rule was never modified after seeing full-benchmark results. Production remains approved v4/v5; V6–V10G are audit-only/nonproduction. Prepared-component semantics, final architecture, and persisted-map migration remain pending, now gated on an architecture reassessment rather than further deterministic tuning. See §5.25, §6, `docs/audits/cooking-mode-v10e-remaining-fn-taxonomy-2026-08-28.md`, `docs/audits/cooking-mode-v10f-lite-active-object-go-no-go-2026-08-28.md`, and `docs/audits/cooking-mode-v10g-active-object-full-frozen-validation-2026-08-28.md`. |
