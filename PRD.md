...
| Mapping Review (queue) | `/mapping-review` (`app/mapping-review/page.tsx`) | Done | **Admin-only** (recipe-admin identity gate). Recipe-level queue for the Cooking Mode human mapping-review workflow — one card per recipe with a persisted mapping proposal, showing derived status (**Needs review / In progress / Ready for final approval / Approved / Stale / Blocked**) and `{resolved}/{total}` progress. Reads via `loadMappingReviewQueue` (`lib/cookingModeMappingReviewQueue.ts`, a bounded unfiltered Firestore collection-group scan over `mappingProposals` — no composite index) behind the admin-gated `GET /api/mapping-review/queue` route. Nav entry in `components/Navigation.tsx`'s "More" sheet/sidebar, filtered to `user?.email === ADMIN_EMAIL`. |
| Mapping Review (recipe detail) | `/mapping-review/[recipeId]` (`app/mapping-review/[recipeId]/page.tsx`) | Done | **Admin-only.** Implements the approved hybrid step-centric review design (`docs/design/cooking-mode-mapping-review-experience-2026-08-28.md`): step-by-step Include/Exclude review of `REVIEW_REQUIRED` candidates (reviewer-vote display, human-readable risk chips from `lib/mappingReviewRiskCopy.ts`, decision correction, on-demand history), an explicit "review complete" milestone (never an implicit auto-advance), a completeness-review screen (`MappingCompletenessPreview` — every accepted relationship incl. `AUTO_ACCEPT`, organized by step, optional desktop grid view), a human-added-missing-relationship picker (`MappingAddRelationshipPicker` — existing ingredient/step rows only, no free text), mandatory map-level completeness attestation, and map approval. All mutations go through admin-verified API routes (below); the page never talks to Firestore directly. **Cooking Mode runtime is unchanged** — approving a map here does not affect what `components/CookingMode.tsx` reads. |
...
| `/api/mapping-review/[recipeId]/relationships` | POST / DELETE | Bearer token (admin required) | Human-added-relationship add/remove. The recipe's current parsed source is always re-derived server-side from the live recipe document, never trusted from the client. Wraps `addHumanMappingRelationship`/`removeHumanMappingRelationship`. |
| `/api/mapping-review/[recipeId]/candidates/[candidateId]/history` | GET | Bearer token (admin required) | On-demand decision history for one candidate (`getMappingReviewHistory`) — default-hidden in the UI, fetched only when a reviewer opens the disclosure. |
...
### `recipes/{recipeId}/mappingProposals/{proposalId}` — Cooking Mode mapping proposals (generation live; Cooking Mode runtime still dormant)
Added 2026-08-28 (Implementation 3) as the durable-persistence layer for the AI-at-ingestion
review architecture in `docs/architecture/cooking-mode-review-routing-contract.md`. **The
`/mapping-review` admin UI (Implementation 5, 2026-08-29) reads and writes these paths** via
admin-verified `/api/mapping-review/**` API routes, and **as of Implementation 6 (2026-08-29) a
proposal is generated and persisted automatically whenever a new shared recipe is published**
(Queue publish, Discover generate-and-save, Discover plan-suggestion save), via the trusted
`POST /api/mapping/generate` route — see the API Routes table and §7 Feature Backlog. But
**Cooking Mode production runtime still does not read any of it**: `components/CookingMode.tsx`
and existing v4/v5 map behavior are completely unchanged, and the current-approved pointer this UI
can now populate is not yet read by anything at request time. Runtime cutover remains a later,
separately gated task. All writes remain admin-only (same trusted boundary as other shared-recipe
writes), via the Admin SDK only — see **Firestore rules** below.
...
- `recipes/{recipeId}/mappingProposals/{proposalId}/candidates/{candidateId}` — one document per
  candidate (`PersistedMappingCandidateV1`), `candidateId` = the existing frozen `mc1:` identity
  (`recipeId`/`recipeRevision`/`ingredientRowIndex`/`stepIndex` — identity is origin-independent).
  `provenance.candidateOrigin` (`'REVIEWER_UNION' | 'HUMAN_ADDED'`, Implementation 4B, 2026-08-29)
  distinguishes an AI-discovered candidate (full reviewer votes, deterministic evidence, routing
  decision/reasons — stored so a later review UI can render/decide without re-running AI) from one a
  human created directly during completeness review for a relationship neither blind reviewer
  proposed (`reviewerA`/`reviewerB: null` — never a fabricated vote; `routingDecision: 'HUMAN_ADDED'`,
  a fourth value alongside the three routing-table outcomes, never produced by the router). The
  materialized `finalDecision`/`decisionSource`/`reviewStatus`/`effectiveReviewEventId` reflect the
  latest effective decision (AUTO at generation time, or the current human decision — including a
  `HUMAN_ADDED` candidate's own initial ACCEPT); the append-only event log below remains the
  authoritative history. See `addHumanMappingRelationship`/`removeHumanMappingRelationship`/
  `listHumanAddedMappingRelationships` (`lib/cookingModeMappingHumanRelationship.ts`).
- `recipes/{recipeId}/mappingProposals/{proposalId}/reviewEvents/{decisionId}` — append-only human
  ...
  (`lib/cookingModeMappingReviewPersistence.ts`) — this exact mechanism also records a `HUMAN_ADDED`
  candidate's creation (a first ACCEPT decision on a freshly created candidate) and its
  correction/removal (a superseding REJECT) or restore (a further superseding ACCEPT); Implementation
  4B introduces no second, parallel event-type system.
- `recipes/{recipeId}/mappingProposals/{proposalId}/completenessAttestations/{attestationId}`
  (Implementation 4B) — immutable map-level completeness attestations
  (`PersistedMappingCompletenessAttestationV1`), new `ma1:` identity prefix, deterministic over
  `(proposalId, reviewStateHash)`. `reviewStateHash` is a hash of every candidate's
  `(candidateId, finalDecision, decisionSource, candidateOrigin)`, so any later candidate addition or
  decision change makes a previously-recorded attestation stop matching the live state — invalidation
  is a pure read-time recomputation, not a tracked event. Required (non-null and current) before
  `buildApprovedMapping` will build a map, for every proposal, unconditionally — resolving the final
  candidate decision never implicitly attests completeness. See
  `recordMappingCompletenessAttestation`/`getMappingCompletenessAttestationStatus`
  (`lib/cookingModeMappingCompletenessAttestation.ts`).
- `recipes/{recipeId}/approvedMappings/{mapId}` — immutable approved maps
  ...
  so exact-replay writes and reruns of the same accepted set always hash identically. Each accepted
  relationship carries `provenanceClass: 'AUTO_ACCEPT' | 'HUMAN_REVIEW_ACCEPT' | 'HUMAN_ADDED'`
  (Implementation 4B, included in `mapHash`), answering "why is this ingredient on this step?" without
  rerunning AI. Never mutated in place; a correction is a new proposal and a new approved map. See
  `buildApprovedMapping` (pure) and `persistApprovedMapping`
  (`lib/cookingModeMappingApprovedPersistence.ts`).
...
15. **Auto-nutrition on publish** — `computeAndStoreNutrition(recipeId, token, timeoutMs)`
    ...
    **(Implementation 6, 2026-08-29)** All three sites now also fire `triggerCookingModeMappingGeneration(recipeId, token)`
    (same file) concurrently with this call via `Promise.allSettled` — see item 22 below. The two
    enrichments are fully independent: either can fail without affecting the other or the recipe's
    own already-completed save.
...
25. **Deterministic cooking-step ingredient mapping foundation** —
    ...
    **2026-08-28 review-routing contract (defined; no implementation):** The normative V1 contract
    is `cooking-review-routing-v1`. Ingredient-step candidates use deterministic IDs bound to
    `recipe-content-v1:sha256:<mappingSourceHash>` revisions; reviewer A/B votes normalize to
    `ACCEPT`/`REJECT`/`UNPARSEABLE`/`MISSING`; human decisions are candidate-level append-only events;
    and approval is a separate map-level, immutable, exact-readback act. The only semantic
    `AUTO_ACCEPT` class is: both complete blind reviewers accept, the exact V1 truth-blind evidence
    extractor completes, and all nine finite V10B risk classes are absent. On the frozen 861-member
    reviewer union this resolves 382 TP / 0 FP (100% observed precision), leaving 479 review items
    across 34/36 recipes. Any disagreement, risk, unsupported class, missing reviewer, or evidence
    failure requires review/blocks approval. `AUTO_REJECT` is structural invalidity only; semantic
    uncertainty and V10G rejection never auto-reject. V10G remains review-priority/diagnostic evidence
    in V1 because its 773-accept combined frontier includes V10D AI-arbiter decisions; prepared
    components are deferred to a separate contract. Same-revision retries are idempotent, approved
    maps never mutate in place, and a parser/source-array change creates a new revision. This contract
    does not authorize activation: the frozen reviewer union still misses 35/868 truth relationships,
    so the complete recall/severity and map-completeness gates remain mandatory. Next implementation
    task: contract types, deterministic identity helpers, frozen evidence adapter, and pure router
    tests only—no Firestore, AI, UI, runtime integration, or activation. See
    `docs/architecture/cooking-mode-review-routing-contract.md` and
    `docs/audits/cooking-mode-review-routing-contract-analysis-2026-08-28.md`.

    **2026-08-28 review-routing core implementation (production domain code; not activated):**
    The canonical V1 TypeScript contracts now live in `types/cookingModeMapping.ts`. Pure,
    provider-neutral modules implement exact source/revision and `mc1:` candidate identity plus
    structural validation (`lib/cookingModeMappingIdentity.ts`), the frozen V10B-facts-to-nine-risk
    adapter (`lib/cookingModeMappingEvidence.ts`), and the three-state precedence table plus canonical
    serialization (`lib/cookingModeMappingRouter.ts`). `computeMappingRecipeRevision` reuses the
    existing exact `JSON.stringify({ ingredients, instructions })` SHA-256 source identity and returns
    `${parserVersion}:sha256:${mappingSourceHash}`; `computeMappingCandidateId` hashes the fixed V1
    identity tuple. The production router reproduces the frozen selected 382 `AUTO_ACCEPT` / 479
    `REVIEW_REQUIRED` result and reserves `AUTO_REJECT` for structural invalidity. V10G evidence/tags
    remain non-authoritative. This slice adds no reviewer execution, AI call, Firestore path/read/write,
...
26. **Cooking Mode mapping generation on ingestion (Implementation 6, 2026-08-29)** —
    `generateAndPersistCookingModeMappingProposal` (`lib/cookingModeMappingIngestion.ts`) is the
    single production entry point from a finalized recipe source to a persisted mapping proposal
    under item 25's proposal/candidate persistence layer. It has no "new recipe vs. edited recipe"
    branch: `recipeRevision` (parser version + SHA-256 of the parsed ingredient/instruction arrays,
    architecture-contract §3) is recomputed from the recipe's *current* `content` every call, so an
    unchanged revision (including a metadata-only "edit," and any exact resubmission) deterministically
    reproduces the same `proposalId` and is durably reused — zero AI calls, `outcome: 'REUSED_EXISTING'`
    — while a changed revision always generates fresh, `outcome: 'GENERATED'`, and never touches the
    prior revision's proposal/candidates/approved map (both remain immutable historical evidence).
    `outcome: 'BLOCKED'` means the proposal generated but has a *non-review* blocking reason (a
    reviewer slot failed, deterministic evidence was unavailable, a structural/source problem) —
    deliberately distinct from the routine "some candidates need human review" case, which is a
    successful `'GENERATED'` outcome that simply needs review next (the same `nonReviewBlockers`
    filter `lib/cookingModeMappingReviewQueue.ts` and `lib/cookingModeMappingStatus.ts` already use).
    `outcome: 'FAILED'` (recipe not found, an exception, an unreconciled persistence readback) never
    throws — every failure mode is reported in the return value. This function never writes
    `cookingModeMappingPointer/current`; that pointer only moves via explicit map approval
    (`app/api/mapping-review/[recipeId]/approve/route.ts`), so an edit can never silently repoint an
    already-approved recipe to an unreviewed proposal — a stale approved pointer simply reads as
    ...
    blocks the other or delays the recipe's own already-completed save. See §3
    `mappingProposals`, §6, and §7 Feature Backlog for the full picture, including the current
    absence of any production recipe-edit UI that would exercise the edit branch of this wiring.

    `getMappingStatusForRecipe` (`lib/cookingModeMappingStatus.ts`) adds a single-recipe read
    (`NO_PROPOSAL | NEEDS_REVIEW | IN_PROGRESS | READY_FOR_FINAL_REVIEW | APPROVED | STALE | BLOCKED`)
    for tests/observability, reusing the existing `loadMappingReviewRecipe` join and the exact
    `MappingReviewQueueStatus` vocabulary the `/mapping-review` queue already renders — not a second
    lifecycle model.
...
- **Cooking Mode's next architecture is reviewed AI-at-ingestion, not another deterministic
  ...
  `docs/audits/cooking-mode-mapping-architecture-reassessment-2026-08-28.md`.
- **The V1 Cooking Mode review-routing contract favors measured precision over the earlier low-review
  projection.** The selected standalone class is both reviewers accepting with complete V1 evidence
  and no V10B risk: 382/861 auto-accepted, 382 TP / 0 FP, while 479/861 require review across 34/36
  frozen recipes (14.09 items per affected recipe). Agreement-only is unsafe (nine FP); allowing
  single-reviewer low-risk candidates gains only four items; and the recorded 773/0 V10G frontier is
  not deployable routing because its V10D baseline used AI arbitration. Also keep precision and
  discovery completeness separate: both reviewers missed 35/868 frozen truth relationships, so
  candidate routing alone does not authorize activation. `AUTO_REJECT` remains structural-only,
...
- **Cooking Mode mapping generation is now wired into recipe ingestion, but no production edit path
  ...
  fields; it never touches the shared `content` field the mapping source is parsed from. The new
  ingestion trigger (`generateAndPersistCookingModeMappingProposal`,
  `lib/cookingModeMappingIngestion.ts`) is therefore exercised today only by the three new-recipe
  sites — its edit-safety (Phase 9-11 of the task: same-revision no-op, different-revision new
  proposal, old approved map/pointer left untouched) is real and tested (`tests/cookingModeMappingIngestion.test.ts`)
  but has no current production caller for the edit case. When a future task adds a shared-content
  edit surface, call the same trigger after that save — do not build a second orchestration path.
  Do not add an edit UI/route as part of resolving this note; it is documentation, not a backlog item.
  See §3 `mappingProposals`, §7 Feature Backlog, and architecture-contract §27.
- **Mapping generation runs concurrently with nutrition on publish, not sequentially, and neither
  ...
  (`MAPPING_GENERATION_TIMEOUT_MS`) — shorter than the route's own `maxDuration = 280` on
  `/api/mapping/generate` — so the publish flow gives up waiting well before the platform would;
  because Vercel serverless functions are not killed by a client disconnect, the server-side
  generation still runs to completion and persists the proposal even if the browser stopped
  waiting for it, so a later `/mapping-review` visit can still discover it. No new UI state was
...
- **`/api/mapping/generate` reuses `verifyAdminToken`, matching every other Cooking-Mode-mapping-
  ...
  second non-admin account could publish a recipe whose mapping never gets generated (client-side
  the trigger call simply 401s, is caught, logged, and ignored — the recipe still publishes fine).
- **Mapping-proposal idempotency is revision-keyed, not "new vs. edit"-branched.** `recipeRevision`
  (and therefore `proposalId`) is a pure function of the recipe's current parsed ingredients/
  instructions/parser version (architecture-contract §3/§16) — `generateAndPersistCookingModeMappingProposal`
  has no "is this a create or an edit" conditional at all. A metadata-only change (or, currently,
  any resubmission of literally unchanged content) reproduces the exact same `proposalId`, so it is
  read back and reused (`REUSED_EXISTING`, zero AI calls) rather than regenerated; only a change to
  the parsed ingredient/instruction arrays or the parser version yields a new `proposalId`, which
  always generates fresh and never touches the prior revision's proposal/candidates/approved map. A
  `WRITING`/`FAILED` header at that exact identity (an interrupted prior attempt) is not trusted and
  falls through to a fresh generation attempt; `saveMappingProposal`'s existing conflict detection
  still protects against two attempts disagreeing on immutable generation content.
- **Pilot corpus remediation is NO-GO because completeness review cannot correct an `AUTO_ACCEPT`
  false positive and the observed review burden is operationally excessive (2026-08-29).** The exact
  8-recipe pilot generated all five fresh same-revision proposals successfully (11 visible reviewer
  attempts, including one recovered Reviewer-B retry), but routed 352/447 reviewer-union candidates
  to human review (78.75%; 50.29 items per affected recipe versus the 14.09 benchmark). Mole Poblano
  alone produced 206 review-required candidates and one authoritative known-truth false positive in
  the `AUTO_ACCEPT` set: ingredient row 14 (`¼ Teaspoonanises seeds`) on instruction index 7, whose
  text acts only on sesame seeds. `/mapping-review` completeness review displays auto-resolved rows
  but exposes only Add, not an override/remove action, and an already-approved recipe has no reopen
  controls; the append-decision service also intentionally accepts only `REVIEW_REQUIRED` or
  `HUMAN_ADDED` candidates. The reused Steak Bites approved map independently contains one known
  false positive (human-added `salt and pepper` on Step 3) that the approved-state UI cannot correct.
  Old v4/v5 maps remained byte-equivalent by audit hash and runtime remained on the old path. Do not
  start remaining-corpus batches. The smallest follow-up is a separately designed, tested successor-
  map correction workflow that can explicitly exclude an `AUTO_ACCEPT` relationship and reopen an
  approved same-revision map without mutating prior immutable artifacts; then rerun this exact pilot
  from its preserved proposals. See
...
| Cooking Mode recall remediation | High | Review-routing core, live reviewer pipeline, mapping persistence, human-added-relationship contract, completeness attestation, human review UI, recipe-ingestion wiring, and end-to-end workflow validation implemented; runtime cutover, edit-triggered remapping (no current call site — see §6), existing-corpus remediation, and activation pending | The selected architecture remains **AI-at-ingestion with review**. Normative V1 routing is exact and implemented as pure production domain code in `types/cookingModeMapping.ts` and `lib/cookingModeMapping{Identity,Evidence,Router}.ts`: both complete blind reviewers accept + complete exact-version V1 evidence + zero finite V10B risks → `AUTO_ACCEPT`; disagreement/risk/failure/unsupported class → `REVIEW_REQUIRED`; verified structural invalidity only → `AUTO_REJECT`. Live-reviewer pipeline validated 2026-08-28 against the real Vercel AI Gateway model (12/12 valid calls, 0 retries, 0 routing mismatches). **Durable persistence — Done, dormant (2026-08-28, Implementation 3)**: `recipes/{recipeId}/mappingProposals/{proposalId}` (+`candidates`/`reviewEvents` subcollections) and `recipes/{recipeId}/approvedMappings/{mapId}` plus a small `cookingModeMappingPointer/current` pointer; see §3. Proposal/candidate writes are atomic-enough via a `WRITING → READY` header status and readback count reconciliation, idempotent on exact replay, and fail closed on any identity conflict (`lib/cookingModeMappingProposalPersistence.ts`). Human review decisions are append-only and supersession-chained (`lib/cookingModeMappingReviewPersistence.ts`). Approved maps are immutable, content-hash-identified (`am1:` prefix), and never mutated in place (`lib/cookingModeMappingApprovedPersistence.ts`); the current-approved pointer only moves after an approved map is persisted and hash-reverified on readback, and is classified `CURRENT`/`STALE`/`NOT_FOUND` against the live recipe revision. **Human mapping-review design approved — 2026-08-28**: hybrid step-centric primary review, `AUTO_ACCEPT` hidden from routine review but visible in a distinct completeness stage, decision history hidden by default, and a mandatory map-level completeness attestation. **Human-added missing relationship — Done, dormant (2026-08-29, Implementation 4B)**: candidate `provenance.candidateOrigin` (`'REVIEWER_UNION' | 'HUMAN_ADDED'`) distinguishes AI-discovered from human-added relationships sharing one canonical `mc1:` identity (never a duplicate at an occupied identity); `addHumanMappingRelationship`/`removeHumanMappingRelationship`/`listHumanAddedMappingRelationships` (`lib/cookingModeMappingHumanRelationship.ts`) reuse the exact append-only `appendMappingReviewDecision` mechanism rather than a second event-type system. Map-level completeness attestation (`recipes/{recipeId}/mappingProposals/{proposalId}/completenessAttestations/{attestationId}`, `ma1:` prefix) is required — unconditionally, for every proposal — before `buildApprovedMapping` will build a map; a deterministic review-state hash makes invalidation (a relationship added/changed, a new revision) a pure recomputation rather than a tracked event (`lib/cookingModeMappingCompletenessAttestation.ts`). Approved relationships now carry `provenanceClass: 'AUTO_ACCEPT' | 'HUMAN_REVIEW_ACCEPT' | 'HUMAN_ADDED'`, included in `mapHash`. See architecture-contract §26. **Human mapping-review UI — Done (2026-08-29, Implementation 5)**: the approved design is implemented at the admin-only `/mapping-review` (queue) and `/mapping-review/[recipeId]` (step-centric review, completeness review, human-added relationships, attestation, approval) routes — see the Page Inventory rows above and the seven new `/api/mapping-review/**` routes, all behind `verifyAdminToken`. `AUTO_ACCEPT` relationships are hidden during routine candidate review and visible (lower-emphasis) during completeness review; review history is default-hidden, fetched on demand; a zero-`REVIEW_REQUIRED` proposal still requires completeness review and attestation and never auto-approves; a mapping mutation after attestation invalidates it (re-attestation required before approval); approval revalidates the live server state so a stale browser can never approve an outdated map. All mutations route through admin-verified API routes calling the exact existing persistence services (no new Firestore access pattern, no client-side Firestore reads/writes) — the admin-only console rules documented under **Firestore rules** already cover every path this UI touches, so **no further manual Console change is required** for this task. `components/CookingMode.tsx` and existing v4/v5 runtime behavior remain completely unchanged; as of Implementation 5, recipe-ingestion did not yet trigger mapping-proposal generation (see the Implementation 6 update below — that gap is now closed for new-recipe finalization). 158 focused tests via an in-memory Firestore test double + mocked API-route/component tests (zero AI calls, zero production writes; 100 from Implementations 3-4B + 58 from Implementation 5). Candidate review remains append-only and candidate-level, approval/completeness remains map-level, retries remain idempotent, and all failures close as not approved. Activation remains blocked by end-to-end recall/severity gates because the frozen reviewer union missed 35/868 truth relationships. Existing-corpus remediation still requires complete human review before immutable-manifest/SHA-locked zero-AI apply. **Recipe-ingestion wiring — Done, dormant with respect to Cooking Mode runtime (2026-08-29, Implementation 6)**: `generateAndPersistCookingModeMappingProposal` (`lib/cookingModeMappingIngestion.ts`) is the single orchestration boundary from a finalized recipe source to a persisted proposal — revision-keyed identity (not a "create vs. edit" branch) makes it naturally idempotent for unchanged content and edit-safe for changed content (old proposal/candidates/approved map immutable, current-approved pointer never touched here — pointer moves only via explicit map approval, §3). Wired into all three `saveRecipe()` call sites (Queue publish, Discover generate-and-save, Discover plan-suggestion save) behind the new admin-verified `POST /api/mapping/generate` route (`recipeId`, optional `expectedRecipeRevision` for conflict detection); runs concurrently with `computeAndStoreNutrition` via `Promise.allSettled`, independently timeout-guarded and never-throwing, so neither blocks the other or the recipe's own save. Same-revision resubmission reuses the existing `READY` proposal with zero AI calls; a `WRITING`/`FAILED` header at that identity is not trusted and regenerates. `getMappingStatusForRecipe` (`lib/cookingModeMappingStatus.ts`) adds a single-recipe status read (`NO_PROPOSAL` plus the existing queue statuses) for tests/observability. No recipe-edit UI currently mutates shared recipe content, so the edit branch of this wiring is tested but has no production caller yet — see §6. 46 new focused tests (zero AI calls, zero production writes). See §5.25, §6, `docs/architecture/cooking-mode-review-routing-contract.md`, `docs/design/cooking-mode-mapping-review-experience-2026-08-28.md`, `docs/audits/cooking-mode-review-routing-contract-analysis-2026-08-28.md`, `docs/audits/cooking-mode-live-reviewer-validation-2026-08-28.md`, and `docs/audits/cooking-mode-mapping-persistence-2026-08-28.md`. **End-to-end workflow validation — Done (2026-08-29)**: a live 3-recipe validation exercised the complete pipeline through real production paths — generation via `POST /api/mapping/generate` (6 live AI Gateway calls; exact idempotent replay with 0 additional calls), the `/mapping-review` queue and step-centric/completeness UI on desktop and mobile, candidate decisions and append-only corrections with visible history, human-added relationships and duplicate protection (`ALREADY_AI_DISCOVERED` — no duplicate at an AI-occupied identity), completeness attestation plus invalidation/re-attestation (deterministic `reviewStateHash` recomputation confirmed, not a tracked event), map approval (2 maps approved: one `HUMAN_ASSISTED` with 14 relationships across all three provenance classes, one `AUTO` with 7), exact-replay immutability, correct current-pointer resolution, and admin-only (`401` unauthenticated/non-admin) authorization on every mutating endpoint. Confirmed via direct code inspection that Cooking Mode runtime and the recipe document's existing `cookingStepIngredientMap` remain completely untouched by the new pointer/approved-map paths. One minor UI defect — candidate decision History rendered oldest-first instead of the design's required newest-first — was found and fixed with a regression test. See `docs/audits/cooking-mode-end-to-end-workflow-validation-2026-08-29.md`. Runtime cutover, existing-corpus remediation, and activation remain pending exactly as before. **Existing-corpus remediation planning — Done (2026-08-29)**: a read-only inventory (237 shared recipes; 229 with a current v4/v5 map; 7 genuinely unresolved; 1 malformed placeholder doc, `maple-roasted-candied-pecans` — already noted in §3's `recipes/{id}` field description as one of the two intentionally-unfilled incomplete documents) and a full batching/gating plan are recorded in `docs/architecture/cooking-mode-existing-corpus-remediation-plan.md` and `docs/audits/cooking-mode-existing-corpus-remediation-planning-2026-08-29.{md,json}`. Scope: 226 of the 229 old-mapped recipes need fresh proposal generation, 1 (`grilled-zucchini-and-summer-squash`) needs only review resumption, and 2 (`garlic-butter-herb-steak-bites-with-potatoes`, `caprese-salad`) are already approved via the end-to-end validation and are recommended to count as remediated. Default batch size 10; an 8-recipe pilot set is defined (5 fresh-generation + the 3 original problem recipes handled by reuse/resume). Migration-manifest contract and SHA-lock defined; runtime cutover tentatively recommends materializing approved relationships back onto the existing `cookingStepIngredientMap` field (reusing the existing SHA-locked field-only apply pattern) over a new pointer-read runtime path, flagged as a decision the eventual cutover task must explicitly reconfirm — iOS/shared-backend compatibility is *not* a factor (the deprecated iOS client is not a compatibility constraint, per §1). Zero AI calls, zero writes beyond read-only inventory queries. No pilot generation has begun. **Pilot corpus remediation — NO-GO (2026-08-29):** all five fresh proposals generated at current source revisions with 11 reviewer attempts and no unrecovered transport/schema failure, but the eight-pilot population measured 447 reviewer-union candidates: 95 `AUTO_ACCEPT` (21.25%) and 352 `REVIEW_REQUIRED` (78.75%), affecting 7/8 recipes at 50.29 review items per affected recipe versus the 14.09 benchmark. Mole Poblano alone has 206 review-required candidates and one authoritative known-truth `AUTO_ACCEPT` false positive (`¼ Teaspoonanises seeds` on the sesame-seed step). The completeness UI cannot override/remove `AUTO_ACCEPT`, and the reused approved Steak Bites map has a separate known false positive (`salt and pepper` on Step 3) with no approved-state correction/reopen path. Therefore only 2/8 recipes remain approved/current, the original zucchini omissions are not yet fixed in an approved map, and the zero-known-FP/all-approved/UX gates fail. Five fresh proposal headers and 405 candidates were preserved; no new review events, attestations, approved maps, or pointers were written. Old v4/v5 runtime maps were unchanged and Cooking Mode still reads them. **Remaining corpus remediation is not authorized and has not started.** Next: design and implement a successor-map correction workflow for explicit `AUTO_ACCEPT` exclusion and approved same-revision reopening, then rerun this exact preserved pilot before any batch 2. See `docs/audits/cooking-mode-pilot-remediation-2026-08-29.{md,json}`. |
...
paste manually after reviewing sibling-app impact). These paths are internal review-workflow/audit
artifacts with no runtime or UI reader yet, so read is scoped to the same admin identity as write
...
match /recipes/{recipeId}/mappingProposals/{proposalId} {
  ...
  match /completenessAttestations/{attestationId} {
    allow read, write: if isRecipeAdmin();
  }
}
...
**Implementation 5 (2026-08-29) confirms this section needs no change.** The `/mapping-review`
admin UI never reads or writes these Firestore paths from the browser — every mutation and read
goes through an admin-verified `/api/mapping-review/**` route using the Admin SDK, the exact trust
boundary these rules already assume. The rules above remain the correct (still undeployed)
defense-in-depth boundary; no new path, no client-side access pattern, and no rules-file/deploy
