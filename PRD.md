...
- `cookingStepIngredientMap?` is the embedded schema-v1 publish-time map for newly created recipes
  ...
  At the 2026-08-26 recovery checkpoint production contained 228 mapped recipes: the original 187 retained their reviewed v4 maps,
  the recovered 41 use deterministic-v5/hybrid-v5 maps, and the remaining eight unresolved Wave 4/5
  documents remain valid without a persisted map and use the runtime fallback. On 2026-08-29 the
  final Cooking Mode closeout selectively replaced this field for exactly Steak Bites, Caprese, and
  Grilled Zucchini with current approved-map-derived schema-v1 values; exact old values remain in the
  SHA-locked rollback manifest. No other recipe map was changed and there is no corpus-wide backfill.
...
### `recipes/{recipeId}/mappingProposals/{proposalId}` — Cooking Mode mapping proposals (generation/review live; selectively promotable)
...
**Cooking Mode production runtime does not read these subcollections directly**: `components/CookingMode.tsx`
continues to read only the validated embedded `cookingStepIngredientMap`. A trusted selective-promotion
operation may verify a current approved pointer/map and materialize that map into the existing runtime
field; unpromoted recipes retain their prior v4/v5 map unchanged. All writes remain admin-only (same trusted boundary as other shared-recipe
...
- `recipes/{recipeId}/approvedMappings/{mapId}` — immutable approved maps
  ...
  rerunning AI. Never mutated in place; a source revision creates a new proposal, while an append-only
  same-revision decision correction invalidates completeness and produces a new superseding approved
  map without overwriting the historical map. See
...
- **The raw two-reviewer union is below the pragmatic automatic-quality floor (2026-08-29).** A
  precommitted 10-recipe held-out sample excluded the eight pilot recipes, all 36 frozen-benchmark
  recipes, and identifiable V10F-Lite challenge recipes; its 294 adjudicated true relationships were
  source-compatible with current live parsed content before any AI call. Twenty production blind-
  reviewer calls completed on attempt 1. Their raw union scored **294 TP / 342 FP / 0 FN: 46.23%
  precision, 100% recall, 63.23% F1**. CRITICAL, HIGH, and seasoning/herb recall were each 100%, but
  every recipe had false positives and per-recipe precision ranged from 36.49% to 68.42%. The error
  pattern was systemic rather than isolated: 147 component-leakage, 111 lifecycle, and 58 overbroad
  collective-reference false positives dominated the sample. Existing exact-revision reviewer
  evidence did discover all six required original-visible-defect relationships, but that does not
  offset the precision failure. Therefore direct automatic-union persistence is **not** authorized;
  production remains on old mappings and runtime migration remains pending. Do not restart the prior
  perfection-oriented deterministic research series automatically. The next product task is
...
- **Cooking Mode ingredient-mapping remediation is closed; promotion is selective/on-demand only
  (2026-08-29).** Corpus-wide remediation and further mapper research are **STOPPED**. The failed
  held-out raw-union/additive results reject automatic bulk rollout, not the review infrastructure.
  New recipes continue to receive mapping proposals; a future recipe or an existing recipe with a
  concrete visible problem may be reviewed/corrected, approved, and then explicitly promoted into
  the existing `cookingStepIngredientMap` runtime field. Every other old recipe remains on its
  current v4/v5 value. Never infer approval as runtime activation, schedule a bulk backfill, or
  resume V11/V12/perfection-threshold research without a new concrete user-visible problem and
  explicit product authorization. The three original defects were fixed and production-verified
  via the exact manifest SHA `eb804ad43b50c42c72f02ab54136ef8d2a5f10a1c84301e4ab7d34a86c512a26`;
  rollback remains exact and hash-guarded. Prepared components remain deferred/separate. See
  `docs/audits/cooking-mode-mapping-remediation-closeout-2026-08-29.md`.
